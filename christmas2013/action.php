...
// include __DIR__ .'/../../api/v1/config/config.php';
...
function checkLogin(){
    ...
    if (!$isLogin) {
        $data['error'] = 1;
        $data['data'] = '<a href="#myModal1" data-toggle="modal"><div class="login">登入</div></a>';
    } else {
        $data['error'] = 0;
...
function send($account, $password, $password_confirm){
    ...
    responseJson(0, '系統將寄認證信至您所設定的電子信箱，請確認後並點選所附的網路連結');
}
